...
export const CardPDF = ({ name, repo, theme }: prfProps) => {
  ...
  return (
    <Document>
      <Page
        style={styles.page}
        size={{ width: 960, height: 1920 }}
        orientation="landscape"
      >
        <View style={styles.view}>
          <ImagePDF style={styles.image} src={themeCard.image[theme]} />
          <Text
            style={{
              color: themeCard.color[theme],
              fontSize: '101px',
              left: '320px',
              top: '54px',
              ...styles.text,
...
function Banner({ name, repo, theme }: prfProps) {
  return (
    <PDFDownloadLink
      style={{ padding: '1.6rem 4rem' }}
      document={<CardPDF repo={repo} name={name} theme={theme} />}
      fileName="card.pdf"
    >
